...
describe('AMMPool Page', () => {
  ...
  it('shows the limited-data banner for a pool without XRP', async () => {
    // Both sides are issued tokens, so isXrpBased is false and the market metrics,
    // the derived USD values and the chart are all suppressed. Without the banner that
    // reads as missing data rather than a deliberate omission.
    mockGetAMMInfo.mockResolvedValue({
      amm: {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('amm_pool_limited_data_label'),
      ).toBeInTheDocument()
    })
    expect(screen.getByText('amm_pool_limited_data_text')).toBeInTheDocument()
    // The chart is gated on the same flag
    expect(screen.queryByTestId('tvl-volume-chart')).not.toBeInTheDocument()
  })

  it('does not show the limited-data banner for an XRP pool', async () => {
    mockGetAMMInfo.mockResolvedValue(mockAmmInfoResponse)
    mockDetectDeleted.mockResolvedValue(null)
    ...
    expect(
      screen.queryByText('amm_pool_limited_data_label'),
