...
class EnableAmendment extends Component {
  ...
  componentDidMount() {
    ...
    EnableAmendment.nameOfAmendmentID(data.instructions.amendment).then(
      (name) => {
        if (name) {
  ...
  render() {
    const { amendmentName, amendmentStatus, minRippledVersion, expectedDate } =
      this.state
    return (
      <>
        {[
