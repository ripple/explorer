...
describe('PaymentChannelClaim: TableDetail', () => {
  it('renders a claim', () => {
    ...
    expect(container.querySelector('[data-testid="claimed"]')).toHaveTextContent(
      'claimed\uE9000.01 XRP (\uE90020.34284 XRP of \uE90070.00 XRP remaining)',
    )
    expect(container.querySelector('[data-testid="channel-amount"]')).not.toBeInTheDocument()
    expect(container.querySelector('[data-testid="renew"]')).not.toBeInTheDocument()
    expect(container.querySelector('[data-testid="close-request"]')).not.toBeInTheDocument()
    expect(container.querySelector('[data-testid="closed"]')).not.toBeInTheDocument()
    unmount()
  })

  it('renders tx with channel being closed', () => {
    const { container, unmount } = renderComponent(mockPaymentChannelClaimClosed)
    expect(container.querySelector('[data-testid="source"]')).toHaveTextContent(
    ...
    expect(container.querySelector('[data-testid="claimed"]')).not.toBeInTheDocument()
    expect(container.querySelector('[data-testid="channel-amount"]')).toHaveTextContent(
      'channel amount\uE90010.00 XRP',
    )
    expect(container.querySelector('[data-testid="renew"]')).not.toBeInTheDocument()
    expect(container.querySelector('[data-testid="close-request"]')).toHaveTextContent(
  ...
  it('renders tx requesting channel be closed but not closing it', () => {
    const { container, unmount } = renderComponent(mockPaymentChannelClaimCloseDenied)
    expect(container.querySelector('[data-testid="source"]')).toHaveTextContent(
      'sourcerH11fDGhbVH5NVXNXkGAMTmfWhUHjCtA3B:2647131528',
    )
    expect(container.querySelector('[data-testid="destination"]')).toHaveTextContent(
  ...
  it('renders tx with destination tag', () => {
    const { container, unmount } = renderComponent(mockPaymentChannelClaimWithDestinationTag)
    expect(container.querySelector('[data-testid="source"]')).toHaveTextContent(
