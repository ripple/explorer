...
describe('OfferCreateSimple', () => {
  it('renders with an expiration and offer', () => {
    ...
    expect(wrapper.find('[data-test="cancel-id"] .value')).toHaveText('#44866443');
    expect(wrapper.find('[data-test="amount-buy"] .value')).toHaveText(
      `1,080,661.95882 CSC.rCSCManTZ8ME9EoLrSHHYKW8PPwWMgkwr`
    );
    expect(wrapper.find('[data-test="amount-sell"] .value')).toHaveText(`1,764.293151 XRP`);
    wrapper.unmount();
  });
  ...
  it('renders', () => {
    ...
    expect(wrapper.find('[data-test="amount-sell"] .value')).toHaveText(`24,755.081083 XRP`);
  });
});
