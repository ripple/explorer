...
const DEXPairs = (props) => {
    ...
    axios
      ...
      .then((tokenRes) => {
        ...
        for (const token of tokenList) {
          if (
            !(
              token.currency.toUpperCase() === currency.toUpperCase() &&
              token.issuer === accountId
            )
          ) {
            promises.push(
  ...
  const renderRow = (pair) => (
    <tr key={`${pair.token}.${pair.issuer}`}>
      <td className="pair">
        <Currency currency={currency} />/<Currency currency={pair.token} />{' '}
        {pair.average.num} {pair.average.unit}
      </td>
